"""
    ...
    Query namespace
    total_docs: total num of docs in namespace
    max_parallel_query_executions
    batch_index
...
def lambda_handler(event, context):
    ...
    if 'namespace_query_summary' not in event.keys():
        ...
        query_from_index = event.get('namespace_query_summary').get('query_from_index')
        total_docs_queried = event.get('namespace_query_summary').get('total_docs_queried')
    ...
    # 4. Create folder in jobDailyDelta with name based on business_date
    # 5. Save to S3 JobDailyDelta with key based on the business_date input

    # Pretend to query docs in namespace
    if event.get('total_docs') - query_from_index < batch_size:
        total_docs_queried += (event.get('total_docs') - query_from_index)
    else:
        total_docs_queried += batch_size

    query_from_index += event.get('max_parallel_query_executions') * batch_size
